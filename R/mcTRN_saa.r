mcTRN_saa<-function(expr, physnet, N=20, ratio=0.8,lambda = 0.1, cores = 4, alpha = 1){
  ...
  r2=matrix(NA,nrow=ngene,ncol=N)
  rownames(r2)=allgenes
  ...
  for (i in 1:N){
    ...
    # build TRN
    trn0=fitTRN(subsample, physnet,lambda,cores,alpha)
    trn = trn0$beta.coefficients[,-1]
    
    # update r2
    r2[,i] = trn0$r2.predVactual
  ...
  # calculate mean r2
  r2.predVactual = rowMeans( r2 )  

  ## clear unused
  rm(tmplate)
  
  # output beta and frequency
  output=list(freq=freq,beta_mean=beta_mean,beta_sd=beta_sd,r2.predVactual=r2.predVactual,r2_distribution=r2,N=N,ratio=ratio)
